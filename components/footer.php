    ...
    <div class="row">
            ...
            <?php
              ...
              if (!empty($footerServices)) {
                  foreach (array_slice($footerServices, 0, 5) as $srv) {
                      echo '<li><a href="service-details.php?slug=' . $srv['slug'] . '">' . $srv['service_name'] . '</a></li>';
                  }
              } else {
      ...
      <div class="col-lg-3 col-md-6 col-sm-6">
        <div class="footer_panel padding_bottom_half bottom20">
          ...
          <p class="whitecolor bottom15"><?php echo $display_settings['office_address'] ?? 'Office address not available' ?></p>
          <p class="whitecolor bottom15">Open: Mon - Fri</p>

          <?php if (!empty($display_settings['whatsapp'])): ?>
            <a href="https://wa.me/<?php echo $display_settings['whatsapp'] ?>" target="_blank" class="button btnprimary btn-sm">WhatsApp Us</a>
          <?php endif; ?>
        </div>
      </div>
