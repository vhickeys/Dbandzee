...
function authCheckBase($allowedRoles = [])
{
    global $userAccess;
    ...
    $userRole = (int) $_SESSION['user_data']['role'];
...
function authCheckById($id, $page, $allowedRoles = [])
{
    global $userAccess;

    // Step 1: Basic login and suspension checks
    if (! isset($_SESSION['user_data']['userId'])) {
        $_SESSION['message'] = "You are not authorized to access this page. Please login.";
        header("location: login.php");
        exit;
    ...
    // Step 2: Role check (if specific roles are required)
    ...
    if (! isset($id) || empty($id) || ! is_numeric($id)) {
        $_SESSION['errorMessage'] = "Invalid or missing ID. You are not authorized to access this page.";
...
function adminAuth()
{
    authCheckBase([ROLE_ADMIN]);
}

function adminCheckById($id, $page)
{
    authCheckById($id, $page, [ROLE_ADMIN]);
}
