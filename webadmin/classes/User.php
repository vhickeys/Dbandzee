...
class User
{
    ...
    public function createUser($full_name, $email, $password, $phone, $address, $role, $profile_photo)
    {
        try {
            ...
            if ($existingUser) {
                echo json_encode(["status" => "error", "message" => "User already exists! Please log in instead."]);
                return;
            }
            ...
            if ($profile_photo && $profile_photo['error'] === UPLOAD_ERR_OK) {
                $uploadDir = "../../assets/img/profile-photos/";
                if (!file_exists($uploadDir)) {
                    mkdir($uploadDir, 0777, true);
                }

                $fileName = time() . "_" . basename($profile_photo['name']);
                $targetFile = $uploadDir . $fileName;

                if (move_uploaded_file($profile_photo['tmp_name'], $targetFile)) {
                    $photoPath = "../../assets/img/profile-photos/" . $fileName;
                } else {
            ...
            // Insert new user
            $insertSql = "INSERT INTO users (full_name, email, password, phone, address, role, profile_photo) 
                      VALUES (?, ?, ?, ?, ?, ?, ?)";
